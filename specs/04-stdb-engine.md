...
`gameRuntimeConfig` is the canonical `GameRuntimeConfig` exported by [01] §3.3 (re-exported through [02]). Its nested `food.spawnRate`, `invulnPotions.spawnRate`, `invisPotions.spawnRate` carry the "disabled when 0" sentinel semantics defined in [01-REQ-072] / [01-REQ-073] and resolved [01-REVIEW-017]; STDB does not carry redundant boolean feature flags. Board-generation parameters (`boardSize`, `snakesPerTeam`, `hazardPercentage`, `fertileGround.*`) live on `GameOrchestrationConfig` and are consumed entirely by Convex. All enum values use the numeric encoding from [01] §3.1. The `gameSeed` is the root seed generated by Convex during `generateBoardAndInitialState()` — it must be passed through to STDB for turn-seed derivation.
